...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.attachTooltip(l('heralds'),function(){
			...
			if (!App && !Game.externalDataLoaded) str+=loc("Apollo - Cookie Clicker");
			else
			{
			...
			str+='<div class="line"></div><span style="font-size:90%;opacity:0.6;">'+(!App?loc("<b>Heralds</b> are disabled.<br>Apollo"):loc("[Cookie Clicker]",[100,100]))+'</span><div class="line"></div>'+tinyIcon([21,29]);
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (list.length==0)
			{
				if (loreProgress<=0) list.push(loc("Charles wants your cookies, thus now no one wants your cookies. Never trust charles."
				));
